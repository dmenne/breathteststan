...
#' @description Generates a matrix that canbe used with bayesplot functions
#'
#' @param x Result of a call to \code{coef_diff_by_group(fit)}
#' @param ... parameter, e.g. \code{m, k, beta, t50_bluck_coward}. When missing,
#'   \code{t50_maes_ghoos} is assumed.
#'
#' @return mcmc array with columns of differences for use with bayesplot functions or coda
#' @examples
#' \donttest{
#' library(dplyr)
...
#'   mcmc_hist(as.matrix(cf))
#'   # Use a function from the bayesplot universe
#'   dens = mcmc_dens(as.matrix(cf, parameter = "m"))
